...
export default function ProfileHeader({
  ...
  return (
    <div className='grid items-start gap-10 lg:grid-cols-[auto_1fr] xl:gap-40'>
      ...
      <div className='grid grid-cols-2 gap-4 xl:grid-cols-4 xl:gap-8'>
        <StatCard value={games} label={t('profile.games')} />

        <StatCard value={wins} label={t('profile.wins')} />

        <StatCard value={losses} label={t('profile.losses')} />

        <StatCard value={`${winPercentage}%`} label={t('profile.winRate')} />
      </div>
    </div>
  );
}
...
function StatCard({ value, label }: { value: number | string; label: string }) {
  return (
    ...
    >
      ...
      <span className='mt-3 text-[14px]'>{label}</span>
    </div>
  );
}
